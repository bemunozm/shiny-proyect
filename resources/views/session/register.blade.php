      ...
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-lg-5 text-center mx-auto">
            ...
            <p class="text-lead text-white">Crea tu cuenta y encuentra tu empleo ideal.</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="card z-index-0">
            ...
            <div class="card-body">
              <form role="form text-left" method="POST" action="{{route('register.store')}}">
                @csrf
                <div class="mb-3">
                  <input type="text" class="form-control" placeholder="Nombres" name="name" id="name" aria-label="Name" aria-describedby="name" value="{{ old('name') }}">
                  @error('name')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
                <div class="mb-3">
                  <input type="text" class="form-control" placeholder="Apellidos" name="last_name" id="last_name" aria-label="last_name" aria-describedby="last_name" value="{{ old('last_name') }}">
                  @error('last_name')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
              ...
              <div class="col-md-8 mb-3">
                  <input type="text" class="form-control" placeholder="Documento" name="document" id="document" aria-label="Documento" value="{{ old('document') }}">
                  @error('document')
                      <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
              </div>
                  <div class="col-md-4 mb-3">
                      <input type="text" class="form-control" placeholder="CV" name="verifier_code" id="verifier_code" aria-label="Dígito verificador" value="{{ old('verifier_code') }}">
                      @error('verifier_code')
                          <p class="text-danger text-xs mt-2">{{ $message }}</p>
                      @enderror
                  </div>
            ...
            <div class="mb-3">
                <input type="text" class="form-control" placeholder="Teléfono" name="phone" id="phone" value="{{ old('phone') }}">
                @error('phone')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="form-check form-check-info text-left">
                  ...
                  @error('agreement')
                    <p class="text-danger text-xs mt-2">Primero, acepte los Terminos y Condiciones, luego intentelo denuevo.</p>
                  @enderror
                </div>
                <div class="text-center">
                  <button type="submit" class="btn bg-gradient-dark w-100 my-4 mb-2">Crear Cuenta</button>
                </div>
                <p class="text-sm mt-3 mb-0">Ya tienes una cuenta? <a href="login" class="text-dark font-weight-bolder">Iniciar Sesion</a></p>
              </form>
            </div>
          </div>
        </div>
